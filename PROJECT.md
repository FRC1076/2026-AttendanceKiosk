...
decodes the QR code, checks the name against a roster, appends a timestamped row
to a Google Sheet, and confirms the scan on a small LCD with a buzzer chirp.
...
| [README.md](README.md) | Short inventory of the repo and a list of planned improvements. |
...
- 70 PNGs are pre-generated in [PihiQRcodes_ready.zip](PihiQRcodes_ready.zip)
...
- Simplify the startup sequence (no monitor/keyboard needed)
- Design a PCB to replace the breadboard
- Build a case around that PCB
- Add remote control so the kiosk can be turned on and off without being
  physically present
